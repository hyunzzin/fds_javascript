...
function toWeirdCase(s) {
  ...
  for (let i = 0; i < str.length; i++) {
    for (let j = 0; j < str[i].length; j++) {
      result += j % 2 === 0 ? str[i][j].toUpperCase() : str[i][j].toLowerCase();
    }
    result += ' ';
  }
  return result;
...
// 6. 핸드폰 요금 고지서에 표시할 전화번호는 개인정보 보호를 위해 맨 뒷자리 4자리를 제외한 나머지를 *으로 바꿔야 한다. 
// 전화번호를 나타내는 문자열 str을 입력받는 hideNumbers 함수를 완성하라 예를들어 s가 ‘01033334444’면 *******4444를 리턴하고, ‘027778888’인 경우는 *****8888을 리턴한다.

function hideNumbers(str) {
  let first = '';
  const last = str.substring(str.length - 4); // slice(-4)도 된다.
...
function nextSqaure(n) {
  const pow = n ** (1 / 2);
  const result = pow === parseInt(pow) ? (pow + 1) ** 2 : 'no';
  return result;
}
