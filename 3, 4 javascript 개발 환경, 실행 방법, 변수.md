...
1. 
2. 변수
- 메모리 주소로 메모리에 직접 접근하는 것은 시스템을 멈추게 할 수도 있는 치명적 오류를 생산할 가능성이 높은 매우 위험한 일???? 메모리 값을 변경할 수 있는데 어떤 부분에서 치명적 오류를 생산할 수 있다는 것일까?

- var 키워드를 붙이지 않으면 초기화가 이루어지지 않는 것 같은데 맞는지? 다른 차이점은 없는지?

- ex. var b = 20;  var b = 10; b는 10이 나온다.
...
초기화 단게를 거치지 않으면 확보된 메모리 공간에 이전에 다른 애플리케이션이 사용했던 값(쓰레기 값)이 남아 있을 수 있다.
